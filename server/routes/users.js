...
router.post('/register', async (req, res) => {
  try {
    const { name, email, password, bankName, accountNumber, accountName, phoneNumber } = req.body;
    ...
    const user = new User({ name, email, password: hashed, bankName, accountNumber, accountName, phoneNumber });
    await user.save();
...
router.put('/profile', auth, async (req, res) => {
  try {
    const { name, bankName, accountNumber, accountName, phoneNumber } = req.body;
    ...
    await user.save();
    res.json({ success: true, user: { id: user._id, name: user.name, email: user.email, bankName: user.bankName, accountNumber: user.accountNumber, accountName: user.accountName, phoneNumber: user.phoneNumber } });
  } catch (error) {
    res.status(500).json({ error: 'Server error' });
  }
});
