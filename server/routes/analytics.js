...
router.get("/:userId", async (req, res) => {
  try {
    ...
    const campaigns = await Giveaway.find({ creatorId: userId }).sort({
      createdAt: -1,
    });
    ...
    const totalRevenue = campaigns.reduce(
      (sum, campaign) => sum + campaign.totalAmount,
      0
    );

    // Get all participants for user's campaigns
    const campaignIds = campaigns.map((campaign) => campaign._id.toString());
    // Find users who have participated in any of these campaigns
    const participants = await User.find({
      participatedCampaigns: { $in: campaignIds },
    });
    ...
    const conversionRate =
      totalParticipants > 0
        ? Math.round((totalFollowers / totalParticipants) * 100)
        : 0;
    ...
    const monthlyGrowth =
      lastMonthParticipants > 0
        ? Math.round(
            ((currentMonthParticipants - lastMonthParticipants) /
    ...
    const totalPlatforms = Object.values(platformData).reduce(
      (sum, count) => sum + count,
      0
    );
    const topPlatforms = Object.entries(platformData)
      .map(([name, count]) => ({
        ...
        participants: count,
        percentage:
          totalPlatforms > 0 ? Math.round((count / totalPlatforms) * 100) : 0,
      }))
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthRevenue = monthCampaigns.reduce(
        (sum, campaign) => sum + campaign.totalAmount,
        0
      );
