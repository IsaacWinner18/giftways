...
export default function GiveawaysPage() {
  ...
  useEffect(() => {
    const fetchGiveaways = async () => {
      ...
      try {
        ...
        setGiveaways([])
        setError(error instanceof Error ? error.message : "An error occurred while fetching giveaways.")
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    const filtered = giveaways.filter((giveaway) => {
      // Filter out campaigns user already participated in
      if (user && ((user as User).participatedCampaigns ?? []).includes(giveaway._id || giveaway.id || '')) {
        return false
      }
      // Search filter
      if (searchTerm) {
        ...
        return (
          ...
          (String(giveaway.creatorName ? giveaway.creatorName : (giveaway.creator ? giveaway.creator : "")).toLowerCase().includes(searchLower))
        )
      }
      return true
    })
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-8">
            Join thousands of participants in exciting giveaways from creators around the world. Complete simple tasks
            and win real money!
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-2xl mx-auto mb-8">
            <div className="text-center">
              <div className="text-3xl font-bold text-purple-600">
                ₦{giveaways.reduce((sum, g) => sum + (g.totalAmount || 0), 0).toLocaleString()}
              </div>
              <div className="text-sm text-gray-600">Total Prize Pool</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-600">{giveaways.length}</div>
              <div className="text-sm text-gray-600">Active Giveaways</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-emerald-600">
                {giveaways.reduce((sum, g) => sum + (g.currentParticipants || 0), 0).toLocaleString()}
              </div>
        ...
        {filteredGiveaways.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">No Giveaways Found</h3>
              ...
              >
                <CardHeader className="pb-4">
                  <div className="flex items-start justify-between mb-3">
                    <Badge
                      variant="secondary"
                      className="bg-emerald-100 text-emerald-800 border-emerald-200 text-xs px-2 py-1"
                    >
                      {giveaway.status}
                    </Badge>
                    <div className="text-right">
                      <div className="text-sm font-medium text-gray-900">⏰ {giveaway.timeRemaining} left</div>
                    </div>
                  </div>
                  <CardTitle className="text-xl font-bold text-gray-900 group-hover:text-purple-600 transition-colors">
                    {giveaway.title}
                  </CardTitle>
                  <CardDescription className="text-gray-600 line-clamp-2">{giveaway.description}</CardDescription>
                  <div className="text-sm text-gray-500 mt-2">
                    Created by <span className="font-medium text-gray-700">{giveaway.creatorName || giveaway.creator}</span>
                  </div>
                </CardHeader>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="text-center p-4 bg-gradient-to-br from-purple-50 to-violet-50 rounded-lg border border-purple-100">
                      <div className="text-2xl font-bold text-purple-900">₦{(giveaway.totalAmount || 0).toLocaleString()}</div>
                    ...
                    <div className="text-center p-4 bg-gradient-to-br from-emerald-50 to-green-50 rounded-lg border border-emerald-100">
                      <div className="text-2xl font-bold text-emerald-900">₦{giveaway.amountPerPerson || 0}</div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-gray-900">
                        {(giveaway.currentParticipants || 0)}/{giveaway.maxParticipants || 0}
                      </span>
                    </div>
                    <Progress value={getProgressPercentage(giveaway.currentParticipants || 0, giveaway.maxParticipants || 0)} />
                    <div className="text-xs text-gray-500 text-center">
                      {Math.max((giveaway.maxParticipants || 0) - (giveaway.currentParticipants || 0), 0)} spots remaining
                    </div>
                  </div>
                  ...
                  {/* Action Button */}
                  <Link href={`/campaign/${giveaway._id || giveaway.id}`} className="block">
        ...
        {user && (
          <div className="text-center mt-16 p-8 bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl border border-purple-100">
            <h3 className="text-2xl font-bold text-gray-900 mb-4">Want to Create Your Own Giveaway?</h3>
            <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
              Launch your own monetized giveaway campaign and grow your social media following while rewarding your
              community.
            </p>
