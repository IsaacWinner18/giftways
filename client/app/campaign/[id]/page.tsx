...
export default function CampaignPage({
  ...
  const [resolvedParams, setResolvedParams] = useState<CampaignParams | null>(
    null
  );
  ...
  useEffect(() => {
    if (
      user &&
      campaignData &&
      (Array.isArray(user.participatedCampaigns)
        ? user.participatedCampaigns
        : []
      ).includes(campaignData._id || campaignData.id || "")
    ) {
  ...
  const canMarkTaskDone = (taskId: string) => {
    return (
      visitedTasks.includes(taskId) &&
      (taskTimers[taskId] === undefined || taskTimers[taskId] <= 0)
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/campaigns/${id}/participants`, {
        method: "POST",
  ...
  const safeArray = (val: unknown, fallback: unknown[] = []) =>
    Array.isArray(val) ? val : fallback;

  const progressPercentage =
    campaignData && safeNumber(campaignData.maxParticipants) > 0
      ? (safeNumber(campaignData.currentParticipants) /
          safeNumber(campaignData.maxParticipants)) *
        100
      : 0;
        ...
        <Card className="max-w-md w-full text-center border-0 shadow-xl bg-gradient-to-br from-emerald-50 to-green-50">
          <CardContent className="pt-8 pb-8">
            ...
            <p className="text-gray-600 mb-6 text-lg leading-relaxed">
              You&apos;ve already joined this giveaway. Funds will be
              distributed automatically when the campaign ends.
            </p>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <CardTitle className="text-3xl font-bold text-gray-900 mb-3">
                      {safeString(campaignData.title, "Untitled Campaign")}
                    </CardTitle>
                    ...
                    <div className="mt-3 text-sm text-gray-500">
                      ...
                      <span className="font-medium text-gray-700">
                        {safeString(campaignData.creatorName, "Unknown")}
                      </span>
                    </div>
                  </div>
                  <Badge
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-base">
                    ...
                    <span className="font-semibold text-gray-900">
                      {safeNumber(campaignData.currentParticipants)} of{" "}
                      {safeNumber(campaignData.maxParticipants)} spots filled
                    </span>
                  </div>
                ...
                <div className="space-y-6">
                  ...
                  {currentStep === 1 && (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {safeArray(campaignData.socialRequirements).map(
                          (requirement, index) => (
                            <div
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <div className="font-medium">
                                      {getActionText(requirement.action)}{" "}
                                      {requirement.displayName}
                                    </div>
                                    <div className="text-sm text-gray-600 capitalize">
                                      on {requirement.platform}
                                    </div>
                                    {visitedTasks.includes(
                                      requirement.id || `requirement-${index}`
                                    ) &&
                  ...
                  {/* Step 2: Review & Submit (now step 2) */}
                  {currentStep === 2 && (
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 mb-4">
                        <div className="w-8 h-8 bg-purple-600 rounded-full flex items-center justify-center text-white font-bold">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="text-sm text-amber-800">
                              ...
                              <p>
                                ...
                                have completed all the required social media
                                tasks and that all information provided is
                                accurate. Providing false information may result
                                in disqualification.
                              </p>
                            </div>
                          </div>
